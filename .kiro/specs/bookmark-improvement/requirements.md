...
5. IF ブックマークが1件以上ある THEN 並び替え機能（配信日順・再生数順・高評価数順・配信者順）が利用できる SHALL
6. WHEN ブックマーク一覧を表示する THEN 各配信の再生数と高評価数が表示される SHALL
...
5. WHEN モバイルでブックマーク一覧を表示する THEN サムネイルサイズが小さく調整され、一覧性が向上する SHALL
...
**ユーザーストーリー:** ユーザーとして、ブックマークした配信の人気度を一目で把握し、効率的に視聴する配信を選択したい

#### 受入基準

1. WHEN ブックマーク一覧を表示する THEN 各配信の再生数が視覚的に表示される SHALL
2. WHEN ブックマーク一覧を表示する THEN 各配信の高評価数が視覚的に表示される SHALL
3. WHEN 統計情報を表示する THEN 数値がカンマ区切りで読みやすくフォーマットされる SHALL
4. WHEN モバイルで表示する THEN 統計情報が適切なサイズで表示される SHALL
5. IF 統計データが利用できない THEN 0として表示される SHALL
