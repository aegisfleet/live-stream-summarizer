...
| F-003 | 配信内容の要約とタグ付け | 文字起こしデータを基に、Gemini APIを利用して配信内容の要約と関連タグの生成を行う。 | ・対象動画の文字起こしデータを、Gemini API (gemini-2.5-flash) に送信する。<br>・プロンプトを工夫し、以下の情報を含む要約とタグを生成させる。<br>  - 配信全体の概要（200字程度）<br>  - 主要なトピックや見どころ（箇条書き3〜5点）<br>  - 配信内容を表すタグ（例: `ゲーム実況`, `雑談`, `歌枠` など3〜5個）<br>・生成された要約、タグ、動画URL、タイトル、配信者名、配信日時、サムネイルURLなどを構造化データ（JSON形式）としてリポジトリ内に保存する。 |
