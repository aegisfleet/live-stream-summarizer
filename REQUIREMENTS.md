...
- **コンテンツ生成:** Gemini API (via [Gemini CLI Action](https://github.com/google-gemini/gemini-cli-action))
- **データソース:**
...
| F-003 | 配信内容の要約 | 文字起こしデータを基に、Gemini APIを利用して配信内容と見どころを要約する。 | ・Gemini CLI Actionを通じて、対象動画の文字起こしデータをGemini APIに送信する。<br>・プロンプトを工夫し、以下の情報を含む要約を生成させる。<br>  - 配信全体の概要（200字程度）<br>  - 主要なトピックや見どころ（箇条書き3〜5点）<br>・生成された要約、動画URL、タイトル、配信者名、配信日時、サムネイルURLなどを構造化データ（JSON形式）としてリポジトリ内に保存する。 |
...
| ID | 機能名 | 概要 | 詳細 |
| :--- | :--- | :--- | :--- |
| F-005 | アーカイブ一覧表示 | 要約済みの配信アーカイブを一覧で表示する。 | ・各配信をカード形式で表示する。<br>・カードには、動画サムネイル、配信タイトル、配信者名、配信日時、要約（概要）を表示する。<br>・一覧は、配信日時の**降順**（新しいものが上）で表示する。<br>・カードをクリックすると、YouTubeの動画ページが新しいタブで開く。 |
