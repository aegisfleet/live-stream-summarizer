...
class ArchiveManager {
        ...
        sortButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                if (this.currentSortKey === sortKey) {
                    ...
                    // When switching keys, always start with descending
    ...
    updateTitle() {
        ...
        if (videoId) {
            ...
            document.title = `Streams by ${streamerName} - ${this.originalTitle}`;
        } else {
    ...
    async loadData() {
        try {
            ...
            this.archiveData.forEach(archive => {
                ...
                if (archive.tags_en) {
                    archive.tags_en.forEach(tag => this.tags.add(tag));
                }
            });
        } catch (error) {
    ...
    toggleWatchLater(videoId, bookmarkIcon) {
        if (this.watchLaterList.has(videoId)) {
            ...
            bookmarkIcon.title = 'Add to Watch Later';
        } else {
            this.watchLaterList.add(videoId);
            bookmarkIcon.classList.add('active');
            bookmarkIcon.title = 'Remove from Watch Later';
        }
    ...
    setupTagFilter() {
        ...
        if (!filterContainer || !selectAllButton || !toggleButton || !collapsibleContainer) {
            console.error('Required DOM elements for tag filter not found:', {
                filterContainer: !!filterContainer,
                selectAllButton: !!selectAllButton,
        ...
        const toggleTags = () => {
            ...
            toggleButton.textContent = isOpen ? 'Close' : 'Show More';
    ...
    updateTagFilter() {
        ...
        relevantArchives.forEach(archive => {
            if (archive.tags_en) {
                archive.tags_en.forEach(tag => visibleTags.add(tag));
            }
        });
    ...
    setupSiteDescriptionToggle() {
        ...
        const toggleDescription = () => {
            ...
            toggleButton.textContent = isOpen ? 'Close' : 'Read More';
    ...
    filterArchives() {
        ...
        this.filteredData = this.archiveData.filter(archive => {
            ...
            const tagMatch = this.selectedTags.size === 0 ||
                             (archive.tags_en && archive.tags_en.some(tag => this.selectedTags.has(tag)));
    ...
    renderArchives(clearGrid = true) {
        ...
        if (!grid) {
            console.error('Archive grid element not found. Element ID: archive-grid');
            return;
        }
    ...
    createArchiveCard(archive) {
        ...
        img.title = 'Go to details page';
        img.addEventListener('click', () => {
            window.location.href = `${getBasePath()}pages/en/${archive.videoId}.html`;
        });
    ...
    _createCardContent(archive) {
        ...
        title.title = 'Go to details page';
        ...
        dateElement.textContent = `Stream Date: ${new Date(archive.date).toISOString().slice(0, 19).replace('T', ' ')}`;

        const duration = document.createElement('p');
        duration.className = 'duration';
        duration.textContent = `Duration: ${formatDuration(archive.duration, 'en')}`;

        const viewCount = document.createElement('p');
        viewCount.className = 'view-count';
        viewCount.textContent = `Views: ${formatNumber(archive.viewCount)}`;

        const likeCount = document.createElement('p');
        likeCount.className = 'like-count';
        likeCount.textContent = `Likes: ${formatNumber(archive.likeCount)}`;

        const streamer = document.createElement('p');
        streamer.className = 'streamer-name clickable-streamer';
        streamer.textContent = `Streamer: ${archive.streamer}`;
        streamer.title = `Filter by streamer: "${archive.streamer}"`;
        ...
        overviewSummary.textContent = archive.overview_en.summary;

        const overviewMood = document.createElement('p');
        overviewMood.className = 'overview-mood';
        overviewMood.textContent = `Mood: ${archive.overview_en.mood}`;
    ...
    _createCardFooter(archive) {
        ...
        copyButton.title = 'Copy title and URL';
        copyButton.addEventListener('click', (e) => {
            e.stopPropagation();
            const shareUrl = `https://aegisfleet.github.io/live-stream-summarizer/pages/en/${archive.videoId}.html`;
        ...
        shareButton.title = 'Share this stream on 𝕏';
        ...
        detailButton.title = 'View details page';
    ...
    _createCollapsibleSection(archive, type) {
        ...
        title.textContent = type === 'highlights' ? 'Highlights:' : 'Tags:';
        ...
        if (type === 'highlights') {
            if(archive.highlights_en) {
                archive.highlights_en.forEach(highlight => {
                    const li = document.createElement('li');
                    li.classList.add('clickable-highlight', 'highlight-item');
                    li.title = `Click to play from ${highlight.timestamp}`;
                    li.addEventListener('click', (e) => {
                        const seconds = timestampToSeconds(highlight.timestamp);
                        window.location.href = `${getBasePath()}pages/en/${archive.videoId}.html?t=${seconds}`;
                    });
