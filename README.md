...
- **PWA (Progressive Web Apps) 対応**
  ...
  - サイトに更新（新しい要約の追加や機能改善など）があると、新しい`CACHE_NAME`（キャッシュのバージョン名）を持つサービスワーカーが生成されます。ブラウザが新しいサービスワーカーを検知すると、新しいバージョンのファイルがバックグラウンドでキャッシュされます。そして、次回以降のアクセス時や、全てのタブが閉じられた後に新しいサービスワーカーが有効化され、古いキャッシュは自動的に削除されます。これにより、ユーザーは常に最新の状態でサイトを閲覧できます。
- **多言語対応（日/英）**
