...
- **PWA (Progressive Web Apps) 対応**
  ...
  - サイトの更新はサービスワーカーによって管理されています。技術的には、サイトのコンテンツが更新されると、新しい`CACHE_NAME`（キャッシュのバージョン名）を持つサービスワーカーが生成されます。ブラウザがこの新しいサービスワーカーを検知すると、更新版のファイルがバックグラウンドでダウンロードされます。ダウンロード完了後、ユーザーは画面下部に表示される「更新」ボタンを押すことで、即座に最新版へアップデートできます。また、更新ボタンを押さずにアプリをバックグラウンドに移動させたり、一度閉じたりした場合でも、次回表示時には自動的に更新が適用されるため、常に最新の状態で利用できます。
- **多言語対応（日/英）**
