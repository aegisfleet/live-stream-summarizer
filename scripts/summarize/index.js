...
4. 共演者（ゲスト）の名前が分からない場合は「ゲスト」と記載する。（ゲストの名前は動画のタイトルや説明欄に記載されていることが多い）
5. 声だけの出演者がいる場合は映像に頼らず音声から状況を判断する。（不正確な内容は出力しない）
6. "00:00"付近は配信開始時の待ち時間になるケースが多いため、配信者がしゃべりだしてから要約を開始する。
7. 概要(overviewのsummary)は200字程度で、配信の全体像が分かるように要約する。
8. 配信の種類や内容に応じて適切なタグを付与する。
9. 見どころ(highlights)は各範囲ごとに最低3以上、目安としては5分間隔で出来るだけ多く抽出する。
- データの順番は動画の内容に沿って時系列で並べる。
10. 見どころのタイムスタンプ(timestamp)は動画の開始からの経過時間を正確に記載する。
