...
- **PWA (Progressive Web Apps) 対応**
  ...
  - サイトの更新はサービスワーカーによって管理されています。技術的には、サイトのコンテンツが更新されると、新しい`CACHE_NAME`（キャッシュのバージョン名）を持つサービスワーカーが生成されます。ブラウザがこの新しいサービスワーカーを検知すると、更新版のファイルがバックグラウンドでダウンロードされます。ダウンロードが完了すると、画面下部に「サイトが更新されました」という通知と「更新」ボタンが表示され、クリックすることで即座に最新版にアップデートできます。この仕組みにより、ユーザーは好きなタイミングで更新を適用できます。
- **多言語対応（日/英）**
