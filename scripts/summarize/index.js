...
2. 全てのフィールドは必須である。
3. 添付されている動画のサムネイル画像から出演者やゲスト、動画の内容を把握する。
4. 出演者の名前が分からない場合は「配信者」と記載する。
5. 共演者（ゲスト）の名前が分からない場合は「ゲスト」と記載する。（ゲストの名前は動画のタイトルに記載されていることが多い）
6. 声だけの出演者がいる場合は映像に頼らず音声から状況を判断する。（不正確な内容は出力しない）
7. "00:00"付近は配信開始時の待ち時間になるケースが多いため、配信者がしゃべりだしてから要約を開始する。
8. 概要(overviewのsummary)は200字程度で、配信の全体像が分かるように要約する。
9. 配信の種類や内容に応じて適切なタグを付与する。
10. 見どころ(highlights)は各範囲ごとに最低3以上、目安としては5分間隔で出来るだけ多く抽出する。
- データの順番は動画の内容に沿って時系列で並べる。
11. 見どころのタイムスタンプ(timestamp)は動画の開始からの経過時間を正確に記載する。
